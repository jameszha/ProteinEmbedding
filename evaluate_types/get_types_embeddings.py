...
def main():
    if len(sys.argv) < 4:
        print("Usage: python3 get_types_embeddings.py <input:action_types.lst> <input:embedding.lst> <output:types_embeddings.lst>")
        exit()

    input_types_file_name = sys.argv[1]
    ...
    action_types = {'activation':0, 'binding':1, 'catalysis':2, 'reaction':3, 'inhibition':4}

    with open(input_types_file_name, 'r') as f:
        for line in tqdm(f):
            line_data = line.split()
            ...
            if (protein_1 in embeddings and protein_2 in embeddings and target in action_types):
                features = embeddings[protein_1] + embeddings[protein_2]
                for i, value, in enumerate(features):
                    features[i] = str(i+1) + ':' + value
                features = ' '.join(features)

                target = action_types[target]
                output_file.write(str(target) + ' ' + features + '\n')
